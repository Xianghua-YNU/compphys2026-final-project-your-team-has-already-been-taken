...
\begin{document}
    ...
    \begin{multicols}{2}%以下部分内容采用双栏排版
		...
		\begin{center}
			\includegraphics[scale=.6]{./fig/euler-angle.png}
			\captionof{figure}{欧拉角（Euler angles），黑色和蓝色圆盘仅为视觉辅助作用，请勿混淆. \label{fig:ueler-angle}}
		\end{center}
		\begin{center}
			\includegraphics[width = 7cm]{./fig/axes.png}
		...
		\begin{center}
			\includegraphics[width = 7cm]{./fig/minimal_axis.png}
		...
		\begin{center}
			\includegraphics[width = 7cm]{./fig/intermediate_axis.png}
		...
		\end{align}
		\begin{center}
			\includegraphics[width = 7cm]{./fig/E_AM_intersection.png}
		...
		\begin{center}
			\includegraphics[width = 8cm]{./fig/phase_portrait.png}
			\captionof{figure}{关于$\psi$与$\theta$的相图，参数使用的是本论文实验部分的实验对象. \label{fig:phase_portrait}}
		\end{center}
		...
		\begin{center}
			\includegraphics[width = 8cm]{./fig/epsilon.png}
		...
		\begin{center}
			\includegraphics[width = 7cm]{./fig/E_euler.png}
			\includegraphics[width = 7cm]{./fig/dE_euler.png}
			\includegraphics[width = 7cm]{./fig/phase_euler.png}
		...
		\begin{center}
			\captionof{table}{算法概述}
			
			\begin{tabular}{>{\small}l >{\footnotesize\raggedright\arraybackslash}p{3.6cm}}
				\toprule
				\textbf{算法} & \textbf{\small 简述} \\
				\midrule
				RK45 &
				显式自适应步长 Runge--Kutta 方法，通过四阶和五阶嵌套公式估计局部误差，并自动调整积分步长，在保证精度的同时提高计算效率。 \\
				
				DOP853 &
				八阶 Dormand--Prince 自适应 Runge--Kutta 方法，具有更高积分精度和更好的误差控制能力，适用于高精度常微分方程数值求解。 \\
				
				Implicit Midpoint &
				二阶隐式中点辛积分方法，能够保持系统的辛结构，在长时间积分过程中具有良好的能量和角动量守恒性能。 \\
				
				Gauss--Legendre 4 (GL4) \cite{2006_Lubich_BOOK,2009_Ernst_BOOK} &
				四阶两阶段 Gauss--Legendre 辛积分方法，兼具高阶精度和辛结构保持特性，适用于刚体动力学等哈密顿系统的长期数值模拟。 \\
				
				Adaptive GL4 + Projection \cite{2006_Lubich_BOOK,2009_Ernst_BOOK} &
				基于四阶 Gauss--Legendre 辛积分，引入自适应步长控制和流形投影技术，在提高积分效率的同时，将数值解投影回守恒流形，以保持系统能量和总角动量等守恒量。 \\
		...
		\begin{center}
			\includegraphics[width = 7cm]{./fig/compare.png}
			\includegraphics[width = 7cm]{./fig/compare_E.png}
			\captionof{figure}{算法对比，在能量和角动量图中我们把角动量中的$\omega_3$放到了里面，高阶自适应步长算法在剧烈变化（指数级增长和衰减）的时候呈现的能量波动以及波动后的能量的变化十分显著且对应关系明确.\label{fig:method_compare}}
		\end{center}
		
		可以看到，RK45和DOP853两个高阶算法出现了显著问题，纵使算法阶数很高，但是在能量守恒方面仍然会受到明显的指数衰减和增长的冲击，从图中可以看出，在系统到达指数增长区间（中间轴定理起效）也就是顶峰的时候，能量会发生显著的波动，即第二张图的成簇波动，在没有指数增长或衰减的情况下，算法表现良好，能量几乎不变，但是一旦到达指数增长或衰减区间，能量会在短时间内剧烈波动后显著降低，整体呈现阶梯式下降，这也就是本微分方程如前所述的特殊之处，我们做出判断：
		...
		\begin{center}
			...
			\includegraphics[width = 8cm]{./fig/final3_E.png}
			\captionof{figure}{长时间后的情况，没有显著区别，并且能量不变.\label{fig:final3}}
		\end{center}
		
		\begin{center}
			\includegraphics[width = 8cm]{./fig/final3_enhanced.png}
			\includegraphics[width = 8cm]{./fig/final3_enhanced_E.png}
		...
		\begin{center}
			...
			\begin{tabular}{>{\small}l >{\footnotesize\raggedright\arraybackslash}p{3.6cm}}
				\toprule
				\textbf{算法} & \textbf{\small 优缺点} \\
				\midrule
				
				Implicit Midpoint &
				优点：计算逻辑简单、算力消耗低、计算速度快，适合短时、低精度的快速试算场景；\newline
				缺点：精度层级低，误差累积效应显著，无法适配长时间仿真、大振幅强振荡的刚体旋转场景，数值稳定性差。\\
				
				Gauss--Legendre 4 (GL4) \cite{2006_Lubich_BOOK,2009_Ernst_BOOK} &
				优点：四阶高精度，保辛效果好，常规场景下数值误差极小，仿真结果可信度高；\newline
				缺点：采用固定步长，算力利用率低。刚体旋转存在剧烈振荡与平缓变化的交替过程，固定极小步长会造成平缓阶段算力浪费，固定大步长会放大振荡阶段误差；同时无投影校正机制，浮点舍入误差会长期累积，超长时间仿真仍会出现微弱守恒量漂移。 \\
				
				Adaptive GL4 + Projection \cite{2006_Lubich_BOOK,2009_Ernst_BOOK} &
				优点：四阶高精度、自适应高效算力分配、严格物理守恒，彻底解决长时间误差累积、大振幅误差放大问题，适配所有复杂刚体旋转仿真场景；\newline
				缺点：需要进行步长迭代、误差估计、约束方程求解，单步计算逻辑复杂、算力成本最高。\\
		...
		\begin{center}
			...
			\captionof{figure}{欧拉角的数值解，隐式中点辛积分误差非常大.\label{fig:euler_all_E}}
		\end{center}
		
		\begin{center}
			\includegraphics[width = 8cm]{./fig/euler_4_E.png}
			\captionof{figure}{去除隐式中间点.\label{fig:euler4}}
		\end{center}
		
		\begin{center}
			\includegraphics[width = 8cm]{./fig/euler_4_E.png}
			\captionof{figure}{进一步去除所有辛方法.\label{fig:euler4}}
		\end{center}
		
		从中可以看出高阶算法在这种情况虽然有局部震荡，但是整体误差远小于辛方法（5~10个量级），可以看到辛方法（即使有高阶算法提升）在面对时间范围不是很大的问题时，其对快速变化仍旧很脆弱，辛算法在阶数上的提升确实能够在一定程度上提升其精度，但是相比于自适应步长高阶算法来说仍然带来了不小的误差（5个量级）.
		...
		主要是因为在前面的结果中，指数增长结合在高阶自适应步长算法中带来的能量误差远大于辛方法，而这里效果相反，这说明快速变化对辛方法和高阶算法带来的影响不是线性的，我依据方法和方程的形式而不同，我们此处没有改变计算的物理模型的本质，仅仅是带入了欧拉角，最后能量波动的结果截然不同.
		
		但是归根结底算法的特征是一致的，高阶算法仍然保证了其高精度性，以及能量误差的单调性，同样也呈现阶梯式下降，辛方法仍是在局部进行波动，不会有明显积累（红色线的误差，有上升的趋势我们认为是算法导致的局部情况，理由是前面有下降后上升的趋势，最后面又有一些下降的趋势）.
