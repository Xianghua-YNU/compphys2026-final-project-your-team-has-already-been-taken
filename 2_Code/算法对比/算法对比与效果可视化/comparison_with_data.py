...
class SimulationConfig:
    ...
    omega0: tuple[float, float, float] = (-1.382012486, 26.73426247, -12.35547447)

    # 固定步长方法使用
    dt_fixed: float = 0.002

    # solve_ivp 自适应方法参数
    rtol: float = 1e-9
    atol: float = 1e-11
    solve_ivp_max_step: float = 0.002
...
def load_omega_data(csv_path: Path = DEFAULT_DATA_PATH) -> tuple[np.ndarray, np.ndarray]:
    ...
    # MATLAB span = 1653, 1765 is inclusive and 1-based.
    span = slice(1600, 1800)
...
def get_data() -> Solution:
    t,y = load_omega_data(DEFAULT_DATA_PATH)
    return Solution("experiment_data", t, y)
...
def run_all_methods(
    ...
    # 7. 引入实验数据和时间范围
    results['experiment_data'] = get_data()
...
def plot_angular_velocity(
    ...
    for i, ax in enumerate(axes):
        ax.plot([t0,t0],bar,'r--',label = 'begin')
        ax.plot([t1,t1],bar,'r.--',label = 'end')
        for name, sol in results.items():
            ax.plot(
                sol.t,
                sol.y[:, i],
